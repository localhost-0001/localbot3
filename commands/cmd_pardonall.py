...
async def ex(args, message, client, invoke):
    ...
    if len(args) < 1:
        ...
        embed.add_field(name="WARNING", value="Command usage: pardonall @member <reason>", inline=False)
...
#        print(type(warningsnum))
        ...
        c.execute('DELETE FROM warnings WHERE id = ? AND memid = ?' , (message.server.id, memid))
        conn.commit()

        embed=discord.Embed(title="Discord", url=config.invite, color=0x2ecc71)
        embed.set_author(name=config.name, url=config.botinv, icon_url=config.leftlogoembed)
        embed.set_thumbnail(url=config.embedthumbnail)
        embed.add_field(name="SUCCESS", value="Pardoned all warnings of <@%s> .\nTotal Warnings: `0? I guess?`" %(memid), inline=True)
        embed.set_footer(text=config.by)
        await client.send_message(message.channel, embed=embed)
#
    try:
        await embeds.pardonalllog(message, client, memid, ' '.join(args[1:]))
    except:
